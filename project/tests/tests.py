...
def serial_select() -> str:
    ...
    if len(port) == 0:
        print(
            'Error: No serial device found. Please connect a microcontroller and try again.')
        sys.exit(1)
    ...
    with serial.Serial(port, SERIAL_PORTNO, timeout=1) as ser:
        for i in range(no_tests):
            ...
            if len(received_outputs) != len(expected_outputs):
                print(
                    f'FAIL: Test {i} - Mismatched output count ({len(received_outputs)} vs {len(expected_outputs)})')
                failed += 1
                continue
