...
class StackMachine
  ...
  def initialize
    ...
    @stackParser = Parser.new("stack parser") do
      @stack = [] #We store our stack here? Maybe we can store variables too?
     
      token(/PRINT/) {|_| :print} #What do we need to add to tokenizer?
      token(/\s+/)
      token(/\d+/) {|m| m.to_i }
      token(/./) {|m| m }
      ...
      rule :token do
        match(:print) { |_|
          puts @stack.pop
          @stack }
        match('+') { |_| @stack << (@stack.pop + @stack.pop)}
        match('*') { |_| @stack << (@stack.pop * @stack.pop)}
        match(Integer) { |a| @stack << a }
      end
    end
  end
  
  def parse string
    puts "=> #{@stackParser.parse string}"
  end
...
sm = StackMachine.new
sm.parse "3 4 5 + * PRINT"

## What should work with solution:
#sm.parse "3 a SET 4 a GET 5 + * PRINT"
